...
const MintPage = () => {
  ...
  const handleSubmitForm = (e) => {
    ...
    if (isValidNFTMetadata()) {
      ...
      const downloadedMetadata = downloadObjectAsJson(
          nftData,
          `nft-metadata-${counter}`
      );
      return;
  ...
  const handleSubmitMetadata = async (e) => {
    ...
    if (metaDataFile) {
      ...
      toast("minting NFT");
      await mutation.mutateAsync(ipfsFileLink);
    }
  };
  ...
  return (
      <MainLayout>
        <Formik
            initialValues={{files: null, name: '', description: '', image: ''}}
            onSubmit={(values) => {
              console.log(values)
              console.log({
